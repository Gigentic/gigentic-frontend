          ...
          <div>
            ...
            <div className="flex items-center mt-1">
              ...
              {matchPercentage !== undefined && (
                <span className="ml-2 text-sm font-medium text-green-500">{matchPercentage}% match</span>
              )}
            </div>
          </div>
        </div>
        <div className="text-right">
          <p className="text-sm">Amount in Escrow: ${amountInEscrow?.toFixed(2) ?? '0.00'}</p>
          <p className="text-sm">Total Amount: ${totalAmount?.toFixed(2) ?? '0.00'}</p>
        </div>
